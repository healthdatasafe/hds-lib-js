...
class CollectorInvite {
  ...
  constructor (collector, eventData) {
    if (eventData.type !== 'invite/collector-v1') { throw new errors_1.HDSLibError('Wrong type of event', eventData); }
  ...
  setEventData (eventData) {
    if (eventData.id !== this.eventData.id) { throw new errors_1.HDSLibError('CollectInvite event id does not match new Event'); }
    this.eventData = eventData;
  }

  async getSharingData () {
    if (this.status !== 'pending') { throw new errors_1.HDSLibError('Only pendings can be shared'); }
    return {
