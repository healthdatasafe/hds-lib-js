...
class CollectorClient {
  ...
  static async create (app, apiEndpoint, requesterEventId, accessInfo) {
    ...
    if (!requesterEvents[0]) { throw new errors_1.HDSLibError('Cannot find requester event in public stream', requesterEvents); }
    const eventData = {
  ...
  async accept (forceAndSkipAccessCreation = false) {
    ...
    if (forceAndSkipAccessCreation) {
      if (!this.accessData?.apiEndpoint || this.accessData?.delete) { throw new errors_1.HDSLibError('Cannot force accept with empty or deleted accessData', this.accessData); }
    } else {
      if (this.status === 'Active') { throw new errors_1.HDSLibError('Cannot accept an Active CollectorClient'); }
      ...
      this.accessData = accessData;
      if (!this.accessData?.apiEndpoint) { throw new errors_1.HDSLibError('Failed creating request access', accessData); }
    }
