...
class CollectorRequest {
  ...
  setContent (content) {
    ...
    if (futureContent.version != null) {
      ...
      if (numV === 0) {
        ...
        if (numV !== this.#version) { throw new errors_1.HDSLibError(`Invalid CollectorRequest content version: ${futureContent.version}`); }
      }
  ...
  createSection (key, type) {
    if (this.getSectionByKey(key) != null) { throw new errors_1.HDSLibError(`Section with key: ${key} already exists`); }
...
function validateString (key, totest) {
  if (totest == null || typeof totest !== 'string') { throw new errors_1.HDSLibError(`Invalid ${key} value: ${totest}`, { [key]: totest }); }
  return totest;
}
...
function vo0ToV1 (v0Data) {
  if (v0Data.app?.data?.forms) {
    if (v0Data.sections) { throw new errors_1.HDSLibError('Cannot mix data.forms & sections', v0Data); }
    v0Data.sections = [];
