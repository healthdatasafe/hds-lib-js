...
class Collector {
  ...
  async #setStatus (status) {
    if (!_a.STATUSES[status]) { throw new errors_1.HDSLibError('Unknown status key', { status }); }
    const event = {
  ...
  async save () {
    if (this.statusCode !== _a.STATUSES.draft) { throw new Error(`Cannot save when status = "${this.statusCode}".`); }
  ...
  async checkInbox () {
    ...
    for (const responseEvent of responseEvents) {
      ...
      if (inviteEvent == null) { throw new errors_1.HDSLibError(`Cannot find invite event matching id: ${responseEvent.content.eventId}`, responseEvent); }
      const updateInvite = {
      ...
      if (errors.length > 0) { throw new errors_1.HDSLibError('Error activating incoming request', errors); }
  ...
  async createInvite (name, options = {}) {
    if (this.statusCode !== _a.STATUSES.active) { throw new Error(`Collector must be in "active" state error to create invite, current: ${this.statusCode}`); }
    const eventParams = {
  ...
  async sharingApiEndpoint () {
    if (this.statusCode !== _a.STATUSES.active) { throw new Error(`Collector must be in "active" state error to get sharing link, current: ${this.statusCode}`); }
    ...
    if (!newSharingApiEndpoint) { throw new errors_1.HDSLibError('Cannot find apiEndpoint in sharing creation request', { result: access, requestParams: params }); }
  ...
  inviteStatusForStreamId (streamId) {
    ...
    if (status == null) { throw new errors_1.HDSLibError(`Cannot find status for streamId: ${streamId}`); }
    return status;
  }
}
