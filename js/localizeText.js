...
function localizeText (textItem) {
  ...
  if (!textItem.en) { throw new errors_1.HDSLibError('textItems must have an english translation', { textItem }); }
...
function validateLocalizableText (key, toTest) {
  if (toTest.en == null || typeof toTest.en !== 'string') { throw new errors_1.HDSLibError(`Missing or invalid localizable text for ${key}`, { [key]: toTest }); }
  for (const optionalLang of supportedLocales) {
    if (optionalLang === 'en') { continue; }
    if (toTest[optionalLang] != null && typeof toTest[optionalLang] !== 'string') { throw new errors_1.HDSLibError(`Missing or invalid localizable text for ${key} languagecode: ${optionalLang}`, { [key]: toTest, languageCode: optionalLang }); }
  }
  return toTest;
}
