...
2. **App Templates** — Consent-based data collection and sharing (Manager, Collector, Invite, Client flows). Includes the **AppTemplate JSON loader** and **custom-field declarations** (template-private streams via `clientData.hdsCustomField`). See [`ts/appTemplates/CUSTOM-FIELDS-AND-SYSTEM.md`](./ts/appTemplates/CUSTOM-FIELDS-AND-SYSTEM.md) for the design reference. (System messaging now flows through the CMC per-collector channel — see `open-pryv.io/components/cmc/IMPLEMENTERS-GUIDE.md`.) **`cmcDataExport`** rides that channel for GDPR / HIPAA data-export requests: `requestDataExport` (patient), `fulfillDataExportRequest` (doctor), `listDataExportRequests` (either side); the convention is documented in data-model `documentation/CUSTOM-FIELDS-AND-SYSTEM.md`.
