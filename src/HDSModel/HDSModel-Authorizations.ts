  ...
  forItemKeys (itemKeys: string[], options: {
    ...
    for (const itemKey of itemKeys) {
      ...
      if (!streamsRequested[streamId]) { // new streamId
        const auth: any = { streamId, level: opts.defaultLevel };
        if (opts.includeDefaultName) {
 ...
 */
function authorizationOverride (level1: string, level2: string): boolean {
 ...
 */
function mixAuthorizationLevels (level1: string, level2: string): string {
