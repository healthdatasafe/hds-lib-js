...
export default class Collector {
  ...
  #cache: {
    ...
    invitesInitialized: boolean;
    invitesInitializing: boolean;
    statusEvent: any;
    sharingApiEndpoint?: string;
    inviteStatusForStreamId?: { [key: string]: string };
  };
  ...
  async #setStatus (status: string): Promise<any> {
    if (!Collector.STATUSES[status]) throw new HDSLibError('Unknown status key', { status });
