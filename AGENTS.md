...
| `formatEventDate(timeSec)`              | Centralised date formatter — uses `resolveAccountPreference('dateFormat')` (account-level HDSProfile value, else the per-app HDSSettings one), falls back to ISO date when neither is available. | [`docs/toolkit.md`](docs/toolkit.md)                   |
...
| Export                                  | What it does                                                                                          | Read more                                              |
|-----------------------------------------|-------------------------------------------------------------------------------------------------------|--------------------------------------------------------|
| `HDSSettings`, `SETTING_TYPES`          | **Per-app** settings, bound to the app's own `baseStream` — each app has a private copy that does **not** interoperate with other apps. Owns `theme`, `displayName`, and the dynamic `preferred-display-*` / `preferred-input-*` keys. Still exposes `preferredLocales`/`timezone`/`dateFormat`/`unitSystem` for unmigrated apps, but **`HDSProfile` is now the source of truth** for those (plan 78 §C 7.1b). | [`docs/settings.md`](docs/settings.md)                 |
| `HDSProfile`, `PROFILE_FIELDS`          | **Account-level**, shared across every app/connection. Identity (displayName, avatar, name, dob, sex, country) **and account preferences** (`preferredLocales`, `timezone`, `dateFormat`, `unitSystem`) on the `profile-preferences` stream. Backed by Pryv events under `profile/*`. `isStored(key)` distinguishes a stored value from a default. | [`docs/settings.md`](docs/settings.md)               |
| `resolveAccountPreference(key)`, `hasAccountPreference(key)` | Read an account preference with the migration-safe precedence: **stored** HDSProfile value > per-app HDSSettings value > default. Use these rather than reading `HDSSettings.get('dateFormat'\|'unitSystem'\|…)` directly — a bare HDSProfile read would let its non-null default mask a real per-app setting. | [`docs/settings.md`](docs/settings.md) |
