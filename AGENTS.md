...
| Export                                  | What it does                                                                                          | Read more                                              |
|-----------------------------------------|-------------------------------------------------------------------------------------------------------|--------------------------------------------------------|
| `appTemplates`                          | Namespace: `AppManagingAccount` / `AppClientAccount` / `Collector` / `CollectorClient` / `CollectorRequest` / `Contact` / `Questionnaire` (Plan 71). The legal/consent + form-template shape used by every HDS app. `CollectorRequest` can pack one or more `Questionnaire` instances (`addQuestionnaire(q \| content)` / `get questionnaires` / `getQuestionnaire(i)` / `removeQuestionnaire(i)`) for first-contact requests that ask access AND deliver a questionnaire at accept time. Static helpers: `Questionnaire.makeRequestEvent(content, streamIds, time?)` builds a ready-to-write `questionnaire/request-v1` event payload; `Questionnaire.writeBundled(connection, request, streamIds)` batches all bundled questionnaires through `connection.api()` post-accept; `Questionnaire.buildAnswerEvent(requestEventId, answers, knownKeys?)` composes the answer event's content + `clientData.related` mirror per the Pryv §7 cross-reference convention. | [`docs/app-templates.md`](docs/app-templates.md) + [`AppTemplates.md`](AppTemplates.md) + [`data-model/documentation/QUESTIONNAIRE.md`](https://github.com/healthdatasafe/data-model/blob/main/documentation/QUESTIONNAIRE.md) |
